...
export function getLidarBoxCorners(detection) {
    const halfLength = (detection?.length || 0) / 2;
    const halfWidth = (detection?.width || 0) / 2;
    const heading = detection?.heading || 0;
    const cosHeading = Math.cos(heading);
    const sinHeading = Math.sin(heading);
    const centerX = detection?.centerX || 0;
    const centerY = detection?.centerY || 0;
    ...
    return localCorners.map((corner) => ({
        x: centerX + (corner.x * cosHeading) - (corner.y * sinHeading),
        y: centerY + (corner.x * sinHeading) + (corner.y * cosHeading)
    }));
}
...
export function buildLidarPointCloud({
    ...
    return {
        points,
        pointStride: POINT_STRIDE,
        renderedPointCount: Math.floor(points.length / POINT_STRIDE),
        sensorStats: createSensorStats(sensorCounts),
        bounds,
        sampleStride,
        returnMode: includeSecondReturn ? 'dual' : 'return1'
    };
}
